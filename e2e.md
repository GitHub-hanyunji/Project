...
## 🔥 왜 InterFuser는 ROS 없이 동작하는가?

InterFuser가 ROS 없이 동작하는 이유는 **End-to-End 구조** 때문입니다.
<img width="760" height="560" alt="image" src="https://github.com/user-attachments/assets/3f1a8f0c-ce75-45ef-81ed-0fd7ed3553e6" />

### 기존 ROS 방식

```
센서 → Perception → Localization → Planning → Control
        (각각 독립된 ROS 노드로 분리, ROS 토픽으로 연결)
```

각 단계가 ROS 토픽을 통해 데이터를 주고받기 때문에 ROS가 반드시 필요하고, 단계가 많아질수록 오차가 누적됩니다.

### InterFuser 방식

```
센서 (Camera + LiDAR)
        ↓  CARLA Python API로 직접 수집
Transformer (InterFuser)
  ├── Encoder : 멀티센서 융합
  └── Decoder : Waypoint 예측
        ↓
steer / throttle / brake
...
Perception · Localization · Planning · Control이 **하나의 Transformer 모델로 압축**되어 있어, 모듈 간 연결(ROS)이 필요 없습니다.

> **한줄 요약:** ROS는 "모듈 연결용"인데, InterFuser는 애초에 모듈이 하나라서 필요 없음

### 비교

| 항목 | 기존 ROS 방식 | InterFuser E2E |
|------|-------------|----------------|
| 구성 | 노드 여러 개 분리 | 모델 하나로 통합 |
| 미들웨어 | ROS 필수 | Python API만으로 충분 |
| 오차 누적 | 단계별 누적 | 단일 포워드 패스로 최소화 |
| 튜닝 복잡도 | 모듈별 개별 튜닝 | End-to-End 학습 |
| 비유 | 레고 블록 여러 개 연결 | 하나로 합쳐진 완성된 머신 |
